...
const getTops = (dom) => {
    ...
    return Array.from(trs).map(tr => {
        ...
        const sessionData =  Array.from(lis).map(li => {
            ...
            agg['speakers'] = Array.from(li.querySelectorAll(".bt-redner-liste strong")).map(strong => strong.textContent);
            return agg;})
...
Promise.all(requests).then(promises => {
    ...
    fs.writeFile('/Users/knut/Desktop/all_topics.json', json, 'utf8', () => console.log("Done Writing! ✍️"));
});
